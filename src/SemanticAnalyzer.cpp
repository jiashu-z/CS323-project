  ...
  switch (symbol->symbolType) {
    ...
    case SymbolType::ARRAY: {
      return "array (dimension:" +
             std::to_string(
                 std::get<ArrayType *>(symbol->symbolData)->remainDepth) +
             ")";
    }
...
bool ifTypeEquiveLence(Symbol *left, Symbol *right) {
  if (right->symbolType == SymbolType::FUNCTION) {
    return ifTypeEquiveLence(
        left, std::get<FunctionType *>(right->symbolData)->returnType);
  } else if (left->symbolType == SymbolType::FUNCTION) {
    return ifTypeEquiveLence(
        std::get<FunctionType *>(left->symbolData)->returnType, right);
  } else if (left->symbolType != right->symbolType) {
    return false;
  } else {
    switch (left->symbolType) {
      ...
      case SymbolType::STRUCT: {
        return checkStructEquivelence(
            std::get<StructType *>(left->symbolData),
...
bool checkBinaryOperatorInternal(Symbol *left, Symbol *right) {
  if (left->symbolType == SymbolType::FUNCTION) {
    return checkBinaryOperatorInternal(
        std::get<FunctionType *>(left->symbolData)->returnType, right);
  } else if (right->symbolType == SymbolType::FUNCTION) {
    return checkBinaryOperatorInternal(
        left, std::get<FunctionType *>(right->symbolData)->returnType);
  } else {
    bool number = (left->symbolType == SymbolType::INT ||
                   left->symbolType == SymbolType::FLOAT) &&
                  (right->symbolType == SymbolType::INT ||
                   right->symbolType == SymbolType::FLOAT);
    bool typeEqual = (left->symbolType == right->symbolType);
    return number && typeEqual;
  }
}

void checkBinaryOperator(SyntaxTreeNode *parentExp, SyntaxTreeNode *leftOperand,
                         SyntaxTreeNode *rightOperand) {
  if (!checkBinaryOperatorInternal(leftOperand->symbol, rightOperand->symbol)) {
...
bool checkStructEquivelence(StructType *left, StructType *right) {
  if (left->fieldType.size() != right->fieldType.size()) {
    ...
    for (std::vector<Symbol *>::size_type i = 0; i < left->fieldType.size();
         ++i) {
...
bool checkArrayEquivelence(ArrayType *left, ArrayType *right) {
  if (left->baseType->symbolType != right->baseType->symbolType) {
    ...
    switch (left->baseType->symbolType) {
      case SymbolType::ARRAY: {
        ArrayType *leftChild =
            std::get<ArrayType *>(left->baseType->symbolData);
        ArrayType *rightCHild =
            std::get<ArrayType *>(right->baseType->symbolData);
        return checkArrayEquivelence(leftChild, rightCHild);
      }
      case SymbolType::STRUCT: {
        StructType *leftChild =
            std::get<StructType *>(left->baseType->symbolData);
        StructType *rightCHild =
            std::get<StructType *>(right->baseType->symbolData);
...
void getArgsVector(SyntaxTreeNode *args,
                   std::vector<Symbol *> &argsSymbolVector) {
  ...
  if (functionSymbol == nullptr) {
    ...
    if (expNode->children.size() == 3) {
      ...
      if (argsExpectNum != argsActualNum) {
        ...
        checkArgsType(functionType->argsType, actualSymbolVector,
                      expNode->firstLine, idNode->attribute_value);
      }
    }
  }
}
...
void insertExtPrimarySymbol(SyntaxTreeNode *extDefNode,
  ...
  SyntaxTreeNode *decList = extDefNode->children[1];

  auto *varDecs = new std::vector<SyntaxTreeNode *>();
  getDecs(decList, varDecs);
  for (auto &varDec : *varDecs) {
    SyntaxTreeNode *idNode = varDec->children[0];
    std::string symbolName = idNode->attribute_value;
...
Symbol *insertFunctionSymbolWithoutArgs(SyntaxTreeNode *funDec,
  ...
  if (hasDefinition) {
    ...
    if (rt != nullptr) {
      checkFunctionArgsType(std::get<FunctionType *>(rt->symbolData),
                            functionType, funDec->firstLine);
    }
    Symbol *definition =
        symbolTable.searchFunctionDefinitionSymbol(functionName);
    if (definition != nullptr) {
      checkFunctionArgsType(std::get<FunctionType *>(definition->symbolData),
                            functionType, funDec->firstLine);
    }
  }
  return symbol;
...
void insertVarListToSymbolTable(FunctionType *functionType,
                                SyntaxTreeNode *varList,
                                SymbolTable &symbolTable) {
  for (std::vector<Symbol *>::size_type i = 0;
       i < functionType->argsType.size(); ++i) {
...
void checkArgsType(std::vector<Symbol *> &expectArgs,
  ...
  for (std::vector<Symbol *>::size_type i = 0; i < expectArgs.size(); ++i) {
    if (!ifTypeEquiveLence(expectArgs.at(i), actualArgs.at(i))) {
      std::string errorMessage =
          "invalid argument type for " + functionName +
          ". expected: " + typeToString(expectArgs.at(i)) +
          ", actual: " + typeToString(actualArgs.at(i));
...
void checkFunctionArgsType(FunctionType *leftFuntionSymbol,
  ...
  if (leftFuntionSymbol->argsType.size() !=
      rightFunctionSymbol->argsType.size()) {
    std::string errorMessage = "unmatched function declaration";
    printErrorMessage(18, firstLine, errorMessage);
  } else {
    for (std::vector<Symbol *>::size_type i = 0;
         i < leftFuntionSymbol->argsType.size(); ++i) {
      if (!ifTypeEquiveLence(leftFuntionSymbol->argsType.at(i),
                             rightFunctionSymbol->argsType.at(i))) {
        std::string errorMessage = "unmatched function declaration";
        printErrorMessage(18, firstLine, errorMessage);
        return;
      }
...
void insertFunctionSymbol(SyntaxTreeNode *funDec, SymbolTable &symbolTable,
  ...
  if (funDec->children.size() == 3) {
    symbol =
        insertFunctionSymbolWithoutArgs(funDec, symbolTable, hasDefinition);
...
SymbolType getExpDataType(Symbol *exp) {
  if (exp->symbolType == SymbolType::FUNCTION) {
    return std::get<FunctionType *>(exp->symbolData)->returnType->symbolType;
  } else {
...
void checkAssignDataType(SyntaxTreeNode *left, SyntaxTreeNode *right) {
  if (getExpDataType(left->symbol) != getExpDataType(right->symbol)) {
    ...
    switch (left->symbol->symbolType) {
      case SymbolType::STRUCT: {
        StructType *leftChild =
            std::get<StructType *>(left->symbol->symbolData);
        StructType *rightChild =
            std::get<StructType *>(right->symbol->symbolData);
      ...
      case SymbolType::ARRAY: {
        ...
        ArrayType *rightChild =
            std::get<ArrayType *>(right->symbol->symbolData);
...
void insertStructDefinitionSymbol(SyntaxTreeNode *structSpecifierNode,
  ...
  if (structDefinitionTable.find(typeName) == structDefinitionTable.end()) {
    ...
    for (std::vector<Symbol *>::size_type i = 0; i < defNodes->size(); ++i) {
      ...
      getDecs(decList, decs);
      for (std::vector<SyntaxTreeNode *>::size_type j = 0; j < decs->size();
           ++j) {
...
void checkReturnType(Symbol *expected, Symbol *actual, int line) {
  if (expected->symbolType != SymbolType::UNKNOWN &&
      actual->symbolType != SymbolType::UNKNOWN) {
    switch (actual->symbolType) {
      case SymbolType::ARRAY: {
        std::string errorMessage =
            "incompatiable return type: function return value should not be "
            "array type";
      ...
      case SymbolType::STRUCT: {
        if (actual->name != expected->name) {
          std::string errorMessage =
              "incompatiable return type:unmatched struct type";
          printErrorMessage(8, line, errorMessage);
        }
        break;
      }
      case SymbolType::FUNCTION: {
        SymbolType functionReturnType =
            std::get<FunctionType *>(actual->symbolData)
...
void preOrderCheckStmt(SyntaxTreeNode *node, Symbol *rtType, int &returnCount,
  ...
  } else if (node->children[0]->attribute_name == "FOR" ||
             node->children[0]->attribute_name == "WHILE") {
    flag = true;
  } else if (node->children[0]->attribute_name == "BREAK") {
    if (!flag) {
      printErrorMessage(19, node->firstLine,
                        "can not break in non-loop statement");
    }
  } else if (node->children[0]->attribute_name == "CONTINUE") {
    if (!flag) {
      printErrorMessage(19, node->firstLine,
                        "can not continue in non-loop statement");
    }
  }
...
void assignFunctionReturnType(SyntaxTreeNode *specifier,
  ...
  FunctionType *functionType =
      std::get<FunctionType *>(functionSymbol->symbolData);
  functionType->returnType = specifier->symbol;
}
...
void initFunction(SymbolTable &symbolTable) {
  std::string readFunctionName = "read";
  FunctionType *functionType = new FunctionType;
  functionType->returnType = new Symbol("$1", SymbolType::INT);
  functionType->hasDefinition = true;
  functionType->functionName = readFunctionName;
  Symbol *symbol =
      new Symbol(readFunctionName, SymbolType::FUNCTION, functionType);
  symbolTable.insertFunctionDefinitionSymbol(readFunctionName, symbol);

  std::string writeFunctionName = "write";
  FunctionType *writeFunctionType = new FunctionType;
  functionType->returnType = new Symbol("$1", SymbolType::INT);
  writeFunctionType->hasDefinition = true;
  writeFunctionType->functionName = writeFunctionName;
  Symbol *arg1 = new Symbol("$1", SymbolType::INT, new IntType);
  writeFunctionType->argsType.push_back(arg1);
  Symbol *writeSymbol =
      new Symbol(writeFunctionName, SymbolType::FUNCTION, writeFunctionType);
  symbolTable.insertFunctionDefinitionSymbol(writeFunctionName, writeSymbol);
}
