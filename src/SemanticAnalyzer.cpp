...
bool ifTypeEquiveLence(Symbol *left, Symbol *right) {
    if (right->symbolType == SymbolType::FUNCTION) {
        return ifTypeEquiveLence(left, std::get<FunctionType *>(right->symbolData)->returnType);
    } else if (left->symbolType == SymbolType::FUNCTION) {
        return ifTypeEquiveLence(std::get<FunctionType *>(left->symbolData)->returnType, right);
...
bool checkBinaryOperatorInternal(Symbol *left, Symbol *right) {
    if (left->symbolType == SymbolType::FUNCTION) {
        return checkBinaryOperatorInternal(std::get<FunctionType *>(left->symbolData)->returnType, right);
    } else if (right->symbolType == SymbolType::FUNCTION) {
        return checkBinaryOperatorInternal(left, std::get<FunctionType *>(right->symbolData)->returnType);
    } else {
        ...
        bool typeEqual =
                (left->symbolType == right->symbolType);
...
void checkReturnType(Symbol *expected, Symbol *actual, int line) {
    if (expected->symbolType != SymbolType::UNKNOWN && actual->symbolType != SymbolType::UNKNOWN) {
        switch (actual->symbolType) {
            ...
            case SymbolType::FUNCTION: {
                SymbolType functionReturnType = std::get<FunctionType *>(actual->symbolData)->returnType->symbolType;
...
void initFunction(SymbolTable &symbolTable) {
    ...
    Symbol *symbol = new Symbol(readFunctionName, SymbolType::FUNCTION, functionType);
    ...
    Symbol *writeSymbol = new Symbol(writeFunctionName, SymbolType::FUNCTION, writeFunctionType);
