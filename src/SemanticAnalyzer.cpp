...
void useFunctionSymbol(SyntaxTreeNode * expNode, SymbolTable &symbolTable){
    ...
    if(symbol==nullptr){
        std::cout << "Error Type " << 2 << " at Line " << idNode->firstLine << ": " << "Undefined function \"" << idNode->attribute_value << "\"." << std::endl;
    }
}
...
void insertPrimarySymbol(SyntaxTreeNode* defNode,SymbolTable &symbolTable){
    ...
    for (int i = 0; i < size; ++i) {
        ...
        if (returnPtr == nullptr) {
            ...
            std::cout << "Error Type " << 3 << " at Line " << dec->children[0]->children[0]->firstLine << ": "
                      << "Redefined variable \"" << dec->children[0]->children[0]->attribute_value << "\"."
                      << std::endl;
        }
    }
}
void insertFunctionSymbolWithoutArgs(SyntaxTreeNode* funDec,SymbolTable &symbolTable){
