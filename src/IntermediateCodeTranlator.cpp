...
void getExtDefs(SyntaxTreeNode *extdefList,
...
void translateFunctionAndPrint(SyntaxTreeNode *program,
  ...
  for (int i = 0; i < extdefs.size(); ++i) {
    ...
    if (extDef->productionEnum == ProductionEnum::EXTDEF_FROM_FUNC) {
      std::vector<IntermediateCode *> &code1 =
          translate_fundec(extDef->children.at(0)->children.at(1), symboltable);
      std::vector<IntermediateCode *> &code2 =
          translate_Compst(extDef->children.at(1), symboltable);
...
std::vector<IntermediateCode *> &translate_fundec(SyntaxTreeNode *fundec,
                                                  SymbolTable &symbolTable) {
  //  std::cout<<__func__<<":"<<__LINE__<<std::endl;
...
std::vector<IntermediateCode *> &translate_fundec_without_args(
    SyntaxTreeNode *fundec, SymbolTable &symbolTable) {
  //   std::cout<<__func__<<":"<<__LINE__<<std::endl;
...
void getParamDecs(SyntaxTreeNode *varList,
...
std::vector<IntermediateCode *> &translate_fundec_with_args(
    SyntaxTreeNode *fundec, SymbolTable &symbolTable) {
  //   std::cout<<__func__<<":"<<__LINE__<<std::endl;
  std::string functionName = fundec->symbol->name;
  IntermediateCode *functionCode = createFunctionDecCode(functionName);
...
std::vector<IntermediateCode *> &translate_Compst(SyntaxTreeNode *compst,
                                                  SymbolTable &symbolTable) {
  // std::cout<<__func__<<":"<<__LINE__<<std::endl;
  ...
  for (int i = 0; i < decs.size(); ++i) {
    std::vector<IntermediateCode *> &codes =
        translate_dec(decs.at(i), symbolTable);
    mergeInterCode(compst, codes);
  }
  for (int i = 0; i < stmts.size(); ++i) {
    std::vector<IntermediateCode *> &codes =
        translate_stmt(stmts.at(i), symbolTable);
    mergeInterCode(compst, codes);
  }
  return compst->selfAndChildrenCodes;
}

std::vector<IntermediateCode *> &translate_dec(SyntaxTreeNode *dec,
                                               SymbolTable &symbolTable) {
  switch (dec->productionEnum) {
    ...
    case ProductionEnum::DEC_FROM_VARDEC: {
      ...
      if (vardec->productionEnum ==
          ProductionEnum::VARDEC_FROM_VARDEC_LB_INT_RB) {
        ...
        if (childrenVardec->productionEnum == ProductionEnum::VARDEC_FROM_ID) {
          ...
          IntermediateCode *intermediateCode =
              createArrayDecCode(varName, stoi(intNode->attribute_value));
...
void getStmtNodes(SyntaxTreeNode *stmtList,
                  std::vector<SyntaxTreeNode *> &stmts) {
...
std::vector<IntermediateCode *> &translate_Exp(SyntaxTreeNode *exp,
                                               SymbolTable &symbolTable,
                                               std::string &place) {
  // std::cout<<__func__<<":"<<__LINE__<<std::endl;
  switch (exp->productionEnum) {
    case ProductionEnum::EXP_FROM_ID_LP_ARGS_RP: {
      ...
      FunctionType *functionType =
          std::get<FunctionType *>(function->symbolData);
      std::vector<std::string> args;
      std::vector<IntermediateCode *> &code1 =
          translate_Args(exp->children.at(2), symbolTable, args);
    ...
    case ProductionEnum::EXP_FROM_INT: {
      IntermediateCode *intermediateCode =
          createConstantCode(place, exp->symbol->name);
      exp->selfAndChildrenCodes.push_back(intermediateCode);
      return exp->selfAndChildrenCodes;
    }
    ...
    case ProductionEnum::DEC_FROM_VARDEC_ASSIGN_EXP: {
      ...
      if (leftExp->productionEnum == ProductionEnum::EXP_FROM_EXP_LB_EXP_RB) {
        ...
        std::vector<IntermediateCode *> &code1 =
            translate_Exp(rightExp, symbolTable, temp);
        std::vector<IntermediateCode *> &code2 =
            translate_Exp(leftExp, symbolTable, temp2);
        mergeInterCode(exp, code1);
        mergeInterCode(exp, code2);
        IntermediateCode *lastCode = code2.back();
        if (lastCode->intermediateCodeEnum ==
            IntermediateCodeType::GET_VALUE_IN_ADDRESS) {
          std::string &arrayAddress = lastCode->op1->var_name_;
          IntermediateCode *code3 =
              createAssignValueInAddressCode(arrayAddress, temp);
          exp->selfAndChildrenCodes.push_back(code3);
        }
        return exp->selfAndChildrenCodes;
      }
      if (leftExp->productionEnum == ProductionEnum::EXP_FROM_ID ||
        ...
        std::vector<IntermediateCode *> &code1 = translate_Exp(
            rightExp, symbolTable, leftExp->children.at(0)->attribute_value);
        mergeInterCode(exp, code1);
        if (place != "null") {
    ...
    case ProductionEnum::EXP_FROM_EXP_BINARY_EXP: {
      ...
      if (exp->children.at(0)->productionEnum != ProductionEnum::EXP_FROM_ID) {
        temp1 = new_temp();
        std::vector<IntermediateCode *> &code1 =
            translate_Exp(exp->children.at(0), symbolTable, temp1);
        mergeInterCode(exp, code1);
      } else {
      ...
      if (exp->children.at(2)->productionEnum != ProductionEnum::EXP_FROM_ID) {
        temp2 = new_temp();
        std::vector<IntermediateCode *> &code2 =
            translate_Exp(exp->children.at(2), symbolTable, temp2);
        mergeInterCode(exp, code2);
      } else {
        temp2 = exp->children.at(2)->children.at(0)->attribute_value;
      }
      IntermediateCode *code3 = createBinaryCode(
          place, temp1, temp2, exp->children.at(1)->attribute_value);
      exp->selfAndChildrenCodes.push_back(code3);
      return exp->selfAndChildrenCodes;
    }
    case ProductionEnum::EXP_FROM_MINUS_EXP: {
      ...
      std::vector<IntermediateCode *> &code1 =
          translate_Exp(exp->children.at(1), symbolTable, temp1);
    ...
    case ProductionEnum::EXP_FROM_NOT_EXP: {
      ...
      std::vector<IntermediateCode *> &code1 =
          translate_cond_Exp(exp, symbolTable, label1, label2);
    ...
    case ProductionEnum::EXP_FROM_LP_EXP_RP: {
      std::vector<IntermediateCode *> &code1 =
          translate_Exp(exp->children.at(1), symbolTable, place);
      mergeInterCode(exp, code1);
      return exp->selfAndChildrenCodes;
    }
    case ProductionEnum::EXP_FROM_EXP_LB_EXP_RB: {
      ...
      std::string temp1 = "";
      std::string temp2 = new_temp();
      std::string arrayAddress = new_temp();
      if (exp->children.at(2)->productionEnum == ProductionEnum::EXP_FROM_ID) {
        temp1 = exp->children.at(2)->attribute_value;
      } else {
        temp1 = new_temp();
        std::vector<IntermediateCode *> &code2 =
            translate_Exp(exp->children.at(2), symbolTable, temp1);
        mergeInterCode(exp, code2);
      }
      IntermediateCode *code1 = createIntMultiF4Code(temp2, temp1);
      IntermediateCode *code3 = createArrayOffsetCode(
          arrayAddress, arrayId, temp2, exp->children.at(1)->attribute_value);
      IntermediateCode *code4 =
          createGetValueInAddressCode(place, arrayAddress);
...
std::vector<IntermediateCode *> &translate_Args(
    SyntaxTreeNode *argsNode, SymbolTable &symbolTable,
  ...
  if (argsNode->productionEnum == ProductionEnum::ARGS_FROM_EXP_COMMA_ARGS) {
    ...
    std::string temp = new_temp();
    std::vector<IntermediateCode *> &code1 =
        translate_Exp(exp, symbolTable, temp);
    argList.insert(argList.begin(), temp);
    std::vector<IntermediateCode *> &code2 =
        translate_Args(argsNode->children.at(2), symbolTable, argList);
    for (int i = 0; i < code1.size(); ++i) {
...
std::vector<IntermediateCode *> &translate_stmt(SyntaxTreeNode *stmt,
                                                SymbolTable &symbolTable) {
  // std::cout<<__func__<<":"<<__LINE__<<std::endl;
  switch (stmt->productionEnum) {
    case ProductionEnum::STMT_FROM_EXP_SEMI: {
      ...
      std::vector<IntermediateCode *> &code =
          translate_Exp(stmt->children.at(0), symbolTable, str);
    ...
    case ProductionEnum::STMT_FROM_RETURN_EXP_SEMI: {
      std::string temp = new_temp();
      std::vector<IntermediateCode *> &code1 =
          translate_Exp(stmt->children.at(1), symbolTable, temp);
    ...
    case ProductionEnum::STMT_IF_LP_EXP_RP_STMT: {
      ...
      std::string label2 = new_label();
      std::vector<IntermediateCode *> &code1 =
          translate_cond_Exp(stmt->children[2], symbolTable, label1, label2);
      IntermediateCode *codelabel1 = createLabelCode(label1);
      std::vector<IntermediateCode *> &code2 =
          translate_stmt(stmt->children[4], symbolTable);
    ...
    case ProductionEnum::STMT_IF_LP_EXP_RP_STMT_ELSE_STMT: {
      ...
      IntermediateCode *codelabel2 = createLabelCode(label2);
      std::vector<IntermediateCode *> &code3 =
          translate_stmt(stmt->children[6], symbolTable);
    ...
    case ProductionEnum::STMT_WHILE_LP_EXP_RP_STMT: {
      ...
      IntermediateCode *codelabel1 = createLabelCode(label1);
      std::vector<IntermediateCode *> &code1 =
          translate_cond_Exp(stmt->children[2], symbolTable, label2, label3);
      IntermediateCode *codelabel2 = createLabelCode(label2);
      std::vector<IntermediateCode *> &code2 =
          translate_stmt(stmt->children.at(4), symbolTable);
    ...
    case ProductionEnum::STMT_FROM_COMPST: {
      std::vector<IntermediateCode *> &code1 =
          translate_Compst(stmt->children[0], symbolTable);
      mergeInterCode(stmt, code1);
      return stmt->selfAndChildrenCodes;
    }
    default:
...
void mergeInterCode(SyntaxTreeNode *merge,
                    std::vector<IntermediateCode *> &codes) {
  //   std::cout<<__func__<<":"<<__LINE__<<std::endl;
...
std::vector<IntermediateCode *> &translate_cond_Exp(SyntaxTreeNode *exp,
                                                    SymbolTable &symbolTable,
                                                    std::string &lb_t,
                                                    std::string lb_f) {
  // std::cout<<__func__<<":"<<__LINE__<<std::endl;
  switch (exp->productionEnum) {
    case ProductionEnum::EXP_FROM_EXP_RELOP_EXP: {
      ...
      IntermediateCode *ifGOTOCode;
      ifGOTOCode = createIfGOTOCode(temp1, temp2,
                                    exp->children.at(1)->attribute_value, lb_t);
    ...
    case ProductionEnum::EXP_FROM_EXP_AND_EXP: {
      std::string label1 = new_label();
      std::vector<IntermediateCode *> &code1 =
          translate_cond_Exp(exp->children.at(0), symbolTable, label1, lb_f);
      IntermediateCode *labelCode = createLabelCode(label1);
      std::vector<IntermediateCode *> &code2 =
          translate_cond_Exp(exp->children.at(2), symbolTable, lb_t, lb_f);
      mergeInterCode(exp, code1);
    ...
    case ProductionEnum::EXP_FROM_EXP_OR_EXP: {
      std::string label1 = new_label();
      std::vector<IntermediateCode *> &code1 =
          translate_cond_Exp(exp->children.at(0), symbolTable, lb_t, label1);
      IntermediateCode *labelCode = createLabelCode(label1);
      std::vector<IntermediateCode *> &code2 =
          translate_cond_Exp(exp->children.at(2), symbolTable, lb_t, lb_f);
      mergeInterCode(exp, code1);
    ...
    case ProductionEnum::EXP_FROM_NOT_EXP: {
      std::vector<IntermediateCode *> &code1 =
          translate_cond_Exp(exp->children.at(1), symbolTable, lb_f, lb_t);
      mergeInterCode(exp, code1);
      return exp->selfAndChildrenCodes;
    }
    default:
