...
FILE *fd;
extern struct linklist *linklist;
extern struct linklist *a0_a3_list;
bool t0_available = true;
bool t1_available = true;
bool t2_available = true;
bool s0_available = true;
bool s1_available = true;
bool s2_available = true;
bool a0_available = true;
bool a1_available = true;
bool a2_available = true;
bool a3_available = true;
...
Register get_register(tac_opd *opd) {
    ...
    struct node *aRegister = findRegister(a0_a3_list, var);
    if (aRegister != NULL) {
        if (aRegister->aRegister == 0) {
            return a0;
        } else if (aRegister->aRegister == 1) {
            return a1;
        } else if (aRegister->aRegister == 2) {
            return a2;
        } else {
            return a3;
        }
    ...
    if (t0_available) {
        t0_available = false;
        reg = t0;
    } else if (t1_available) {
        t1_available = false;
        reg = t1;
    } else if (t2_available) {
        t2_available = false;
        reg = t2;
    } else {
        reg = t3;
    }
    _mips_iprintf("lw %s, array + %d #### variable: %s, data offset: %d", _reg_name(reg), offset, opd->char_val,
                  offset);
    return reg;
}
...
Register get_register_w(tac_opd *opd) {
    ...
    if (s0_available) {
        s0_available = false;
        return s0;
    } else if (s1_available) {
        s1_available = false;
        return s1;
    } else if (s2_available) {
        s2_available = false;
        return s2;
    } else {
        return s3;
...
Register get_register_t5(tac_opd *opd) {
    return t5;
}

Register get_register_t6(tac_opd *opd) {
    return t6;
}
...
tac *emit_assign(tac *assign) {
    ...
    if (_tac_quadruple(assign).right->kind == OP_CONSTANT) {
        _mips_iprintf("li %s, %d #### %s = %d", _reg_name(x),
                      _tac_quadruple(assign).right->int_val, _tac_quadruple(assign).left->char_val,
                      _tac_quadruple(assign).right->int_val);
    } else {
        y = get_register(_tac_quadruple(assign).right);
        _mips_iprintf("move %s, %s #### %s = %s ", _reg_name(x), _reg_name(y), _tac_quadruple(assign).left->char_val,
                      _tac_quadruple(assign).right->char_val);
    }
    _mips_iprintf("sw %s, array + %d #### store variable:%s", _reg_name(x), global_write_offset,
                  _tac_quadruple(assign).left->char_val);
    restore_reg();
    return assign->next;
}
...
tac *emit_add(tac *add) {
    ...
    _mips_iprintf("sw %s, array + %d #### store variable:%s", _reg_name(x), global_write_offset,
                  _tac_quadruple(add).left->char_val);
    restore_reg();
    return add->next;
}
...
tac *emit_sub(tac *sub) {
    ...
    if (_tac_quadruple(sub).r1->kind == OP_CONSTANT) {
        ...
        _mips_iprintf("addi %s, %s, %d ######## %s = %d - %s", _reg_name(x),
                      _reg_name(y),
                      _tac_quadruple(sub).r1->int_val,
                      _tac_quadruple(sub).left->char_val,
                      _tac_quadruple(sub).r1->int_val,
                      _tac_quadruple(sub).r2
        );
    } else if (_tac_quadruple(sub).r2->kind == OP_CONSTANT) {
        y = get_register(_tac_quadruple(sub).r1);
        _mips_iprintf("addi %s, %s, -%d ######## %s = %s - %d", _reg_name(x),
                      _reg_name(y),
                      _tac_quadruple(sub).r2->int_val,
                      _tac_quadruple(sub).left->char_val,
                      _tac_quadruple(sub).r1->char_val,
                      _tac_quadruple(sub).r2->int_val
        );
    } else {
        y = get_register(_tac_quadruple(sub).r1);
        z = get_register(_tac_quadruple(sub).r2);
        _mips_iprintf("sub %s, %s, %s ######## %s = %s - %s", _reg_name(x),
                      _reg_name(y),
                      _reg_name(z),
                      _tac_quadruple(sub).left->char_val,
                      _tac_quadruple(sub).r1->char_val,
                      _tac_quadruple(sub).r2->char_val
        );
    }
    _mips_iprintf("sw %s, array + %d #### store variable:%s", _reg_name(x), global_write_offset,
                  _tac_quadruple(sub).left->char_val);
...
tac *emit_mul(tac *mul) {
    ...
    _mips_iprintf("mul %s, %s, %s", _reg_name(x),
                  ...
                  _reg_name(z));
    _mips_iprintf("sw %s, array + %d #### store variable:%s", _reg_name(x), global_write_offset,
                  _tac_quadruple(mul).left->char_val);
    restore_reg();
    return mul->next;
}
...
tac *emit_div(tac *div) {
    ...
    _mips_iprintf("sw %s, array + %d #### store variable:%s", _reg_name(x), global_write_offset,
                  _tac_quadruple(div).left->char_val);
    restore_reg();
    return div->next;
}
...
tac *emit_addr(tac *addr) {
    ...
    _mips_iprintf("sw %s, array + %d #### store variable:%s", _reg_name(x), global_write_offset,
                  _tac_quadruple(addr).left->char_val);
    restore_reg();
    return addr->next;
}
...
tac *emit_fetch(tac *fetch) {
    ...
    _mips_iprintf("sw %s, array + %d #### store variable:%s", _reg_name(x), global_write_offset,
                  _tac_quadruple(fetch).left->char_val);
    restore_reg();
    return fetch->next;
}
...
tac *emit_iflt(tac *iflt) {
    ...
    if (_tac_quadruple(iflt).c1->kind == OP_CONSTANT) {
        x = get_register_t5(_tac_quadruple(iflt).c1);
    ...
    if (_tac_quadruple(iflt).c2->kind == OP_CONSTANT) {
        y = get_register_t6(_tac_quadruple(iflt).c2);
...
tac *emit_ifle(tac *ifle) {
    ...
    if (_tac_quadruple(ifle).c1->kind == OP_CONSTANT) {
        x = get_register_t5(_tac_quadruple(ifle).c1);
        _mips_iprintf("li %s, %d", _reg_name(x),
    ...
    if (_tac_quadruple(ifle).c2->kind == OP_CONSTANT) {
        y = get_register_t6(_tac_quadruple(ifle).c2);
        _mips_iprintf("li %s, %d", _reg_name(y),
...
tac *emit_ifgt(tac *ifgt) {
    ...
    if (_tac_quadruple(ifgt).c1->kind == OP_CONSTANT) {
        x = get_register_t5(_tac_quadruple(ifgt).c1);
        _mips_iprintf("li %s, %d", _reg_name(x),
    ...
    if (_tac_quadruple(ifgt).c2->kind == OP_CONSTANT) {
        y = get_register_t6(_tac_quadruple(ifgt).c2);
        _mips_iprintf("li %s, %d", _reg_name(y),
...
tac *emit_ifge(tac *ifge) {
    ...
    if (_tac_quadruple(ifge).c1->kind == OP_CONSTANT) {
        x = get_register_t5(_tac_quadruple(ifge).c1);
        _mips_iprintf("li %s, %d", _reg_name(x),
    ...
    if (_tac_quadruple(ifge).c2->kind == OP_CONSTANT) {
        y = get_register_t6(_tac_quadruple(ifge).c2);
        _mips_iprintf("li %s, %d", _reg_name(y),
...
tac *emit_ifne(tac *ifne) {
    ...
    if (_tac_quadruple(ifne).c1->kind == OP_CONSTANT) {
        x = get_register_t5(_tac_quadruple(ifne).c1);
        _mips_iprintf("li %s, %d", _reg_name(x),
    ...
    if (_tac_quadruple(ifne).c2->kind == OP_CONSTANT) {
        y = get_register_t6(_tac_quadruple(ifne).c2);
        _mips_iprintf("li %s, %d", _reg_name(y),
...
tac *emit_ifeq(tac *ifeq) {
    ...
    if (_tac_quadruple(ifeq).c1->kind == OP_CONSTANT) {
        x = get_register_t5(_tac_quadruple(ifeq).c1);
        _mips_iprintf("li %s, %d", _reg_name(x),
    ...
    if (_tac_quadruple(ifeq).c2->kind == OP_CONSTANT) {
        y = get_register_t6(_tac_quadruple(ifeq).c2);
        _mips_iprintf("li %s, %d", _reg_name(y),
...
tac *emit_call(tac *call) {
    /* COMPLETE emit function */
    _mips_iprintf("addi $sp, $sp, -20 #### fixed");
    _mips_iprintf("sw $ra, 0($sp)");
    _mips_iprintf("sw $a0, 4($sp) #### fixed");
    _mips_iprintf("sw $a1, 8($sp) #### fixed");
    _mips_iprintf("sw $a2, 12($sp) #### fixed");
    _mips_iprintf("sw $a3, 16($sp) #### fixed");
...
tac *emit_param(tac *param) {
    ...
    while (_tac_kind(pointer) == PARAM) {
        ...
        if (findRegister(a0_a3_list, varName) == NULL) {
            insertParam(a0_a3_list, varName, paramCount);
        }
