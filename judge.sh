...
make_file() {
  ...
  for i in "${able[@]}"; do
    ...
    bin/splc ./${TEST_DIR}/test_1_r${i}.spl > ./${TEST_DIR}/test_1_r"${i}".test.out 2>&1

    diff ./${TEST_DIR}/test_1_r"${i}".test.out ./${TEST_DIR}/test_1_r"${i}".out -y --suppress-common-lines
# when diff_rt is 0, there is no difference; when diff_rt is 1, there are some difference; 2 means diff run error.
    ...
    if [ $diff_rt == 0 ]; then
        ...
        echo "test_1_r${i} have difference. Difference are the above: [Actual|Expected]"
    fi
    echo "========================================================================="
  done
}
